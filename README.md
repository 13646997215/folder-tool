...
- 🔧 修复手机端无法选择文件夹的核心Bug（iOS/微信 webkitdirectory 不生效）
- 📱 新增手机端多文件选择回退方案，支持无路径信息的文件分析
- 📱 新增 iOS/Android/微信平台精确检测与自动降级
- 🔧 修复 processFiles 函数对空 webkitRelativePath 的崩溃问题
- 🔧 修复拖放单个文件被静默忽略的问题
- 🔧 修复 traverseDirectory 未保留 lastModified 的问题
- ⚡ 使用防抖优化文件大小过滤器（minSize/maxSize）
- ⚡ 添加加载动画 30 秒安全超时，防止永久加载
- ✨ 移动端文件处理完成后自动关闭侧边栏
- ✨ 文件输入框重置，支持重复选择同一文件夹
- ✨ 拖放支持降级方案（dataTransfer.files 回退）
- ✨ 拖放无文件时显示友好提示
